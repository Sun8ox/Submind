...
export default function PublicProfilePage({ params }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/users/get/public/${userId}`)
      .then(res => res.json())
      .then(data => {
        if (data.success) {
          setUser(data.user);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center bg-gradient-to-br from-blue-50 to-indigo-100 py-10">
      <div className="bg-white/90 shadow-xl rounded-2xl p-8 w-full max-w-xl mb-8">
        ...
        <div className="mb-2 text-sm text-gray-500">Joined: {user.createdAt ? new Date(user.createdAt).toLocaleDateString() : "—"}</div>
      </div>
      {(videos && videos.length > 0) && (
        <div className="w-full max-w-3xl">
          <h2 className="text-2xl font-semibold text-indigo-700 mb-4">Videos</h2>
          <table className="w-full bg-white rounded-xl shadow overflow-hidden">
            ...
            <tbody>
              {videos.map(video => (
                <tr className="border-t" onClick={() => window.location.href = `/player/${video.id}`} key={video.id}>
